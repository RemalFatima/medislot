-- Allow staff walk-ins through the same booking RPC (source = admin).
-- Anon callers still cannot set source to admin.
...
revoke all on function public.book_appointment(uuid, uuid, timestamptz, text, text, text, public.appointment_source) from public;
grant execute on function public.book_appointment(uuid, uuid, timestamptz, text, text, text, public.appointment_source) to anon, authenticated;
